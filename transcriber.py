...
async def chat(text):
    ...
    async for part in await client.chat(model='mistral', messages=[message], stream=True, options={
        "num_keep": 5,
        ...
        "typical_p": 0.7,
        "repeat_last_n": 33, # Sets how far back the model looks to prevent repetition. The default value is 64.
                ...
                if response.status_code == 200:
                    ...
                    if response_text not in ["Vielen Dank für's Zuschauen.", "Vielen Dank für's Zuschauen!", "SWR 2020", "Vielen Dank.", "Untertitel im Auftrag des ZDF für funk, 2017"]:
                        ...
                        if len(transcription) > 1:
                            asyncio.create_task(process_transcription(transcription))
                            print(transcription)
                else:
                    print(response.json())
